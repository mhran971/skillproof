...
import { 
  ...
  AlertTriangle, CheckCircle, Clock, ListChecks 
} from 'lucide-react';
...
export default function Create({ availableSkills, difficulty_levels }: Props) {
  ...
  const { data, setData, post, processing, errors } = useForm({
    ...
    requirements: '',
    deliverables: '',
    evaluation_criteria: [] as { name: string; weight: number }[],
  ...
  return (
    <CompanyLayout>
      <Head title={t('company.create_challenge')} />
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t('company.create_challenge')}</h1>
            <p className="text-gray-500 text-sm mt-1">{t('company.create_challenge_desc')}</p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border p-6">
            ...
            <div className="space-y-5">
              <div>
                ...
                <input type="text" value={data.title} onChange={(e) => setData('title', e.target.value)} className={inputClass('title')} placeholder={t('company.title_placeholder')} />
                ...
                <div>
                  ...
                  <select value={data.difficulty_level} onChange={(e) => setData('difficulty_level', e.target.value)} className={inputClass('difficulty_level')}>
                    {difficulty_levels.map((level) => (
                      <option key={level} value={level}>{t(`company.difficulty_${level}`)}</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('company.duration_hours')}</label>
                  <input type="number" value={data.duration_hours} onChange={(e) => setData('duration_hours', e.target.value)} className={inputClass('duration_hours')} placeholder="48" min="1" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('company.max_participants')}</label>
                  <input type="number" value={data.max_participants} onChange={(e) => setData('max_participants', e.target.value)} className={inputClass('max_participants')} placeholder="∞" min="1" />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea value={data.description} onChange={(e) => setData('description', e.target.value)} rows={4} className={textareaClass('description')} placeholder={t('company.description_placeholder')} />
              ...
              <div>
                ...
                <textarea value={data.requirements} onChange={(e) => setData('requirements', e.target.value)} rows={4} className={textareaClass('requirements')} placeholder={t('company.requirements_placeholder')} />
              ...
              <div>
                ...
                <textarea value={data.deliverables} onChange={(e) => setData('deliverables', e.target.value)} rows={4} className={textareaClass('deliverables')} placeholder={t('company.deliverables_placeholder')} />
              ...
              {criteria.map((criterion, index) => (
                <div key={index} className="flex items-center gap-3">
                  <input
                    ...
                    placeholder={t('company.criterion_name')}
                  ...
                  {criteria.length > 1 && (
                    <button type="button" onClick={() => removeCriterion(index)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                      <X size={16} />
                    </button>
                  )}
                </div>
            ...
            <div className="flex flex-wrap gap-2">
              {availableSkills.map((skill) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${data.required_skills.includes(skill.id) ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
                >
                  {skill.name}
                </button>
              ))}
          ...
          <div className="bg-white rounded-xl shadow-sm border p-6">
            ...
            <div className="space-y-5">
              <div>
                ...
                <textarea value={data.reward_description} onChange={(e) => setData('reward_description', e.target.value)} rows={2} className={textareaClass('reward_description')} placeholder={t('company.reward_placeholder')} />
              </div>
              <div>
